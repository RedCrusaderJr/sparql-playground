...
import De9imHelper from '/de-9im-helper.mjs';
const D9H = new De9imHelper();

export default function relateFunction(wktGeom1, wktGeom2, patternMatrix) {
	if(!wktGeom1 || !wktGeom2 || !patternMatrix) throw new Error("geof:relate -> One of arguments was null/undefined -> wktGeom1: " + wktGeom1 + ", wktGeom2: " + wktGeom2 + ", patternMatrix: " + patternMatrix);
	wktGeom1 = "" + wktGeom1;
	wktGeom2 = "" + wktGeom2;

	try {
		const markLogicRegion1 = convertWktToMarkLogicGeom(wktGeom1);
		const markLogicRegion2 = convertWktToMarkLogicGeom(wktGeom2);

		const parsedDe9im = D9H.parse(geo.regionDe9im(markLogicRegion1, markLogicRegion2))
		return parsedDe9im === patternMatrix;
...
function convertWktToMarkLogicGeom(wktString) {
